...
import ALLOWEDCOINTYPES from "../constants/coinTypes";
import db from "../db";
import { ChainInfo } from "../types/wallet";

export const getAllChains = async() => {
    const storedChains: Record<string, ChainInfo> = await db.get(db.CHAINS) as unknown as Record<string, ChainInfo>;
